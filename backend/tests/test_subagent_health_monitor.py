...
Covers:
- Running task protection: NEVER interrupt a running task
- Premature stop detection: recover stopped tasks without terminal markers
- Reactivation flow: mark interrupted → restart with recovery prompt
...
class TestRunningTaskProtection:
    ...
    def test_does_not_interrupt_pending_task(
        ...
        jsonl = tmp_session_dir / "task-001.jsonl"
        _write_jsonl(jsonl, [{"ts": "1", "role": "ai", "content": "working"}])

        result = _make_result(task_id="task-001", status=mock_executor_module.status.PENDING)
        mock_executor_module.background_tasks["task-001"] = result
